function photon_dev_group = makeSidePhotonDevices(chip_tag, hide_GC_holes)
...
photon_dev_group = Group([0,0],[1;0],[0;1],{});
origin = [0; 0];
curr_x = origin(1);
curr_y = origin(2);
...
if chip_tag == 'A' || chip_tag == 'B'
  ...
  N_mirror = [24 24 24 17 17];
  device.N_mirror = 500;
  device.N_ramp_in = 10;
  device.N_ramp_out = device.N_ramp_in;
  ampl = 0.110*ones(1,5);
  device.period = 0.420;
  device.wg_width = 0.328; % turns out 290 nm
  device.width_corr = false;
  device.ramp_type = 'lin';
  type = {'sine', 'sine', 'sine', 'sine', 'sine'};
elseif chip_tag == 'C'
  % grating coupler parameters
  P.scaling_pitch = 1.0;
  P.GC_hole_corr = true;
  device.cavity_length = 420;
  P.total_wg_length = 220;
  % reflector parameters
  N_mirror = [6 6 5 4 0];
  device.N_ramp_in = 10;
  device.N_ramp_out = device.N_ramp_in;
  ampl = [0.100 0.100 0.100 0.100 0.100];
  device.period = 0.436;
  device.wg_width = 0.372; % comes out 330 nm
  device.width_corr = true;
  device.duty_cycle = 0.70;
  device.ramp_type = 'lin';
  type = {'rect', 'rect', 'rect', 'rect', 'rect'};
end

device_counter = 6;
for ii = 1:length(N_mirror)
  device.type = type{ii};
  device.ampl = ampl(ii);
  ...
  photon_dev = makeFishboneCavityDevice(curr_x, curr_y, GC_fn, device, P, L, B);
  photon_dev_group.add(photon_dev);
...
if B.generate_CAD
    ...
    %photon_dev_group.todxf('dev_group_test');
