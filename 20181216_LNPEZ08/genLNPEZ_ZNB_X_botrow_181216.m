function [LNPEZ_ZNB_X_Row, L_ZNBX] = genLNPEZ_ZNB_X_botrow_181216()
...
% 42 devices in total
paramsNBZZ = genParamStructs('scale_NB', [1.02:0.0041:1.12],...
    'w_end', 0.26, 'tether_w2', [2.4],...
    'tether_L',[3.5],'tether_w_tether', [1],'w2',0.05,...
    'isGenBB',false);
params_ZZ = genParamStructs('d_nb2refl', [1.85, 1.95,1.85, 1.95, 1.95, 1.85, 1.95, 1.85],...
...
paramsPEZ1 = genParamStructs('w_mid', [0.5,0.55,0.6,0.65],...
    'w_end', 0.26,...
     'g_mid', [0.25],'zz_w',[0.45],'g_metal',[0.15],...
     'offset',circshift([0,0.2,0.3,0.35,0.4,0.5,0.6],2));
 
paramsPEZ2 = genParamStructs('L', [15,20,10],'L2',[5,10],'g', [0.6,0.3,0.6,0.3,0.3,0.6,0.3,0.6]);
paramsPEZ2 = [paramsPEZ2 paramsPEZ2 paramsPEZ2 paramsPEZ2 paramsPEZ2];
paramsPEZ = copyfield(paramsPEZ1, paramsPEZ2);

%% params for ZNB X
paramsZNBX1 = genParamStructs('scale_NB', [0.905:0.0051:1.0],...
    'w_end', 0.26, 'tether_w2', [2.4],...
    'tether_L',[3.5],'tether_w_tether', [1],'w2',0.05,...
    'isGenBB',false, 'isGenPS',false,'theta',-90);

paramsZNBX2 = genParamStructs('L',[5,10,14,10,5,14],'d_nb2refl', [2.3,2.2,2.25],'L2',0,'g',0.3,...
    'w',0.45,'g_metal',0.15);
...
gaps = [0.22,0.17,0.15,0.19,0.16,0.25];
gaps = num2cell(repmat(gaps, 1, 40));
...
%%
LNPEZ_ZNB_X_Row=gpack.Group(0,0,{LNPEZ_Row,LNZNB_X_Row,LNNBZZ_Row,feed_link});
