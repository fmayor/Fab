classdef nanobeam_DS_EC_ZZ_X_probes < gpack.Group
...
methods
    function obj = nanobeam_DS_EC_ZZ_X_probes(varargin)
        obj@gpack.Group(0,0,{});
...
%         w_tetherArm = 30;   % mvoed to obj.P_tether as a parameter
        ...
        rect_CRR = Rect(0,-y_pos,2.1*w_rect_sub,h_rect_sub, 'base','center');
        rect_CRR.layer = obj.layer_medium;
        rect_CRR2 = Rect(0,-y_pos,2.8*w_rect_sub,h_rect_sub, 'base','center');
        rect_CRR2.layer = obj.layer_holes;
        ...
        gholder = gpack.Group(0, - (obj.l_NB_unscaled + 2*obj.P_mirror.a/3) * obj.scale_NB/2,...
            {tether,rect_BLL,rect_BLL2,rect_BLR,rect_BRR,rect_BRL,rect_BLR2,rect_BRR2,rect_BRL2,rect_CRR,rect_CRR2, tri3, tri4, tether_cure, c3, c4});        
        if obj.P_sharp.isGen
        ...
        pad3=genBondPad(xpad3,0,h_pad,w_pad);
        pads = gpack.Group(0,0,{pad1,pad2,pad3});
        pads.layer = obj.layer_metal_BB;
        if obj.isBB
            obj.addelement(pads);
        end
        %make thin helper wire connecting pads which might help during release
        helper=genBondPad(xpad2,0,200,2);
        if obj.isBB
            obj.addelement(helper);
        end
        ...
        w_wire = 7;
        d2pad=10;
        v1=port_B_RL+[1;0]; v2=[xpad1/2;0]; v3 =[v2(1);-w_pad/2-d2pad];
        v4=[xpad2;v3(2)]; v5=[v4(1);0];
        w2=Wire({v1,v2,v3,v4,v5},w_wire);  %left right
        v1=port_B_RR; v2 = [xpad1;v1(2)];
        w1=Wire({v1,v2},w_wire-1); %left mid
        v1=port_B_RL+[1;param_elec.coeff_x3-param_elec.coeff_x];
        v2=[v1(1);w_pad/2+d2pad];
        v3=[xpad2;v2(2)];
        v4=[v3(1);0];
        w3=Wire({v1,v2,v3,v4},w_wire);%left left
        ...
        v2=v1+[0;10];v3 = [xpad1/2;0];
        w3=Wire({v1,v2,v3},w_wire); %right left
        v1=port_B_BR+[-1;0]; v2 = v1+[-7;-10]; v3=[v2(1);-w_pad/2-2*d2pad]; 
        v4=[xpad3;v3(2)]; v5=[v4(1);0];
        w4=Wire({v1,v2,v3,v4,v5},w_wire); %right mid
        v1=port_B_BL+[1.2;0];v2=[v1(1);-w_pad/2-3*d2pad-w_wire/2];
        v5=[xpad3-h_pad/2-d2pad-40;v2(2)];v6=v5+[0;w_pad+3*d2pad+d2pad+w_wire/2];
        v7=[xpad2;v6(2)]; v8=[v7(1);0];
        w5=Wire({v1,v2,v5,v6,v7,v8},w_wire); %right right
...
%             'base','center');
        ...
        %M2 expo box (added 20190226)
        rect_wf_M2L = Rect(-10, -25 -7 + w_wf/2-w_wf/2, w_wf, w_wf,...
            'base','center');
        rect_wf_M2L.layer = 'M2_Field';
        obj.addelement(rect_wf_M2L);
        rect_wf_M2R = Rect(-10, -25 -7 + w_wf/2+w_wf/2, w_wf, w_wf,...
            'base','center');
        rect_wf_M2R.layer = 'M2_Field';
        obj.addelement(rect_wf_M2R);
    end
